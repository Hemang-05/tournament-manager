...
export default function RegisterClient({ tournament, organiserName, initialTeamsCount, slug }: RegisterClientProps) {
  ...
  const [registeredTeamsCount, setRegisteredTeamsCount] = useState(initialTeamsCount);
  ...
  useEffect(() => {
    if (tournament) {
      const limit = tournament.players_per_team || 8;
        ...
        <div className="border-b border-gray-100 pb-5 mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 text-center sm:text-left">
          ...
          <div className="flex-shrink-0 self-center">
            <span className={`px-4 py-2 rounded-xl text-sm font-extrabold border ${
              ...
              Teams Registered: {registeredTeamsCount} / {tournament.max_teams}
            </span>
          </div>
        </div>
